...
extension SwiftyTesseract {
  ...
  public func performOCRPublisher(on data: Data) -> AnyPublisher<String, Swift.Error> {
    Deferred {
      Future { [weak self] promise in
        promise(self?.performOCR(on: data) ?? .failure(SwiftyTesseract.Error.imageConversionError))
      }
    }
    .eraseToAnyPublisher()
  }
